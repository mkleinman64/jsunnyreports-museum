...
public class LoggerFour {

    public LoggerFour(Settings settings) {
       this.settings = settings;
       this.max_watt = settings.getPropertyFloat("logger4.maxwatt");
       this.dirLocation = settings.getProperty("logger4.outputlocation");
       this.PULSE_KWH = settings.getPropertyInt("logger4.pulses");
       if ( PULSE_KWH == 0 ) {
          PULSE_KWH = 2000;
       }
    ...
    public void doLog(Calendar currentPulse) {
        ...
        if ( lastPulseReceived == null || ( currentPulse.get(Calendar.DAY_OF_YEAR) != lastPulseReceived.get(Calendar.DAY_OF_YEAR ) ) ) {
           watt = 0.0f;
            ...
            if ( millisecondsDifference == 0 ) {
                ...
                if ( watt > 250 ) {
                    // more then 250W out of the Gridfit is impossible. theoretical limit = 250W
                    insertlog = false;
                }
            }
        ...
        if ( insertlog ) {
          ...
          System.out.println( "Logger4 : " + toWrite );
