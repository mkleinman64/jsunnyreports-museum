...
import nl.mk.jsunnyreports.loaders.inverterdataloaders.processors.EnphaseFileProcessor;

import org.apache.log4j.Logger;

/**
 * Enphase csvfile parser, (***.csv )
 *
 *
 ...
 * @since     1.4.1.0
 */
public class EnphaseDataLoader extends BaseLoader implements LoaderInterface {

    public EnphaseDataLoader(BaseInverter inverter, InverterData inverterData, Files fileCache, Settings s) {
        super(inverter, inverterData, fileCache, s);
    }

    private static final Logger log = Logger.getLogger(EnphaseDataLoader.class);
    ...
    private void csvFileRemove(File csvFile) {
        ...
        //"2013-04-29 09:30:00 +0200;6168"
        //"2013-04-29 09:35:00 +0200;6242"
        try {
    ...
    private void processFiles(boolean init, Integer year ) {
        ...
        for ( FileEntry f: fe.getFileList() ) {
            ...
            if ( f.isToLoad() ) {
                Runnable enphaseData = new EnphaseFileProcessor(newFile, inverterData, baseInverter, settings, init, year,f.getFc() );
                executor.execute(enphaseData);
            }
