...
import nl.mk.jsunnyreports.loaders.inverterdataloaders.processors.SunesyFileProcessor;

import org.apache.log4j.Logger;

/**
 * Sunesy logfile parser, (***.csv )
 *
 *
 ...
 * @since     1.1.2.2
 */
public class SunesyDataLoader extends BaseLoader implements LoaderInterface {

    public SunesyDataLoader(BaseInverter inverter, InverterData inverterData, Files fileCache, Settings s) {
        super(inverter, inverterData, fileCache, s);
    }

    private static final Logger log = Logger.getLogger(SunesyDataLoader.class);
    ...
    private void processFiles( boolean init, Integer year ) {
        ...
        for ( FileEntry f: fe.getFileList() ) {
            ...
            if ( f.isToLoad() ) {
                Runnable fileData = new SunesyFileProcessor(newFile, inverterData, baseInverter, settings, init, year, f.getFc() );
