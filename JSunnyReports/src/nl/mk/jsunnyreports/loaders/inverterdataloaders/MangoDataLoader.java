...
import nl.mk.jsunnyreports.loaders.inverterdataloaders.processors.MangoFileProcessor;
 ...
 * @since     1.1.2.4
 */
public class MangoDataLoader extends BaseLoader implements LoaderInterface {
    private static final Logger log = Logger.getLogger(MangoDataLoader.class);

    public MangoDataLoader(BaseInverter inverter, InverterData inverterData, Files fileCache, Settings s) {
    ...
    private void csvFileRemove(File csvFile) {
        ...
        // 2016/01/31 09:35:50.515
        formatter = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss.SSS");
        formatter.setTimeZone(Constants.getLocalTimeZone());

        try {
            CSVReader reader = new CSVReader(new BufferedReader(new FileReader(csvFile)), ',', CSVWriter.DEFAULT_QUOTE_CHARACTER, 1 );
    ...
    private void processFiles(boolean init, Integer year ) {
        ...
        for ( FileEntry f: fe.getFileList() ) {
            ...
            if ( f.isToLoad() ) {
                Runnable fileData = new MangoFileProcessor(newFile, inverterData, baseInverter, settings, init, year, f.getFc() );
