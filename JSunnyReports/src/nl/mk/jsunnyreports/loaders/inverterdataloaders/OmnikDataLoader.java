...
import nl.mk.jsunnyreports.loaders.inverterdataloaders.processors.OmnikFileProcessor;
 ...
 * @since     1.4.1.0
 */
public class OmnikDataLoader extends BaseLoader implements LoaderInterface {

    private static final Logger log = Logger.getLogger(OmnikDataLoader.class);

    public OmnikDataLoader(BaseInverter inverter, InverterData inverterData, Files fileCache, Settings s) {
    ...
    private void csvFileRemove(File csvFile) {
        ...
        try {
            ...
            // we are reading the whole file, not necessary, bit excessive. but it works.
    ...
    private void processFiles(boolean init, Integer year ) {
        ...
        for ( FileEntry f: fe.getFileList() ) {
            ...
            if ( f.isToLoad() ) {
                Runnable fileData = new OmnikFileProcessor(newFile, inverterData, baseInverter, settings, init, year,f.getFc() );
